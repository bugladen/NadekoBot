                    ...
                    try
                    {
                        ...
                        else if (!before.Roles.SequenceEqual(after.Roles))
                        {
                            if (before.Roles.Count() < after.Roles.Count())
                            {
                                ...
                                str += $"👤`{before.ToString()}` **User's Role Added** 🆔 `{before.Id}`\n⚔⏫ {string.Join(", ", diffRoles)} `Now has:` **{string.Join(", ", after.Roles.Select(r => r.Name)).SanitizeMentions()}** `role(s).`";
                            }
    ...
    `New:` {((ITextChannel)after).Topic}").ConfigureAwait(false);
            ...
            private Task _client_MessageDeleted(ulong arg1, Optional<IMessage> imsg)
            {
                ...
                var task = Task.Run(async () =>
                {
                    try
                    {
                        var str = $@"🕔`{prettyCurrentTime}`👤`{msg.Author.Username}#{msg.Author.Discriminator}` **Deleted Message** 🆔 `{msg.Author.Id}` #⃣ `{channel.Name}`
